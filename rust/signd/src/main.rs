...
use lanzaboote_signd::handlers::Handlers;
use lanzaboote_signd::policy::TrivialPolicy;
use lanzaboote_signd::route;
...
fn serve(args: ServeCommand) -> Result<()> {
    ...
    rouille::start_server(format!("0.0.0.0:{}", args.port), route(handlers));
}
