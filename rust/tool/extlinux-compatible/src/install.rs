...
use crate::esp::ExtlinuxEspPaths;
use crate::extlinux::ExtlinuxConfig;
...
impl Installer {
    pub fn new(
        ...
        let esp_paths = ExtlinuxEspPaths::new(esp);
    ...
    pub fn install(&mut self) -> Result<()> {
        ...
        let extlinux_config = self.install_generations_from_links(&links)?;
        self.install_extlinux_configuration(extlinux_config)?;
        ...
        if self.broken_gens.is_empty() {
            ...
            // deleted).
            self.gc_roots
                .collect_garbage_with_filter(&self.esp_paths.extlinux, |p| {
                    p.file_name()
                        .and_then(|n| n.to_str())
    ...
    /// generations.
    fn install_generations_from_links(&mut self, links: &[GenerationLink]) -> Result<ExtlinuxConfig> {
        ...
        let extlinux_config = self.build_generation_artifacts_from_links(
            &mut generation_artifacts,
            links,
        ...
        F: FnMut(&mut Self, &Generation, &mut GenerationArtifacts) -> Result<()>,
    {
        ...
        let mut extlinux_config = Vec::new();
        ...
        for generation in generations {
            ...
            extlinux_config.push(generation.clone().into());
            ...
            for (name, bootspec) in &generation.spec.bootspec.specialisations {
                ...
                extlinux_config.push(specialised_generation.clone().into());
            }
        }
    ...
    /// Install extlinux configuration to ESP.
    ///
    /// This is only necessary if the system does not support UEFI
    /// and therefore does not rely on EFIVARS or EFI fallback paths
    /// to boot.
    ///
    /// It is also useful if you want to use U-Boot as a bootloader,
    /// this is important given kernel-provided DTB can fail to be loaded
    /// in UEFI.
    fn install_extlinux_configuration(&self, config: ExtlinuxConfig) -> Result<()> {
        // Create a temporary file of extlinux.conf
        let mut extlinux_file = tempfile::NamedTempFile::new()?;
        // Render `config` in it
        for entry in config {
            writeln!(&mut extlinux_file, "{}", entry.to_string())?;
        }
        // Install it.
        install(
            extlinux_file.path(),
            &self.esp_paths.extlinux_config
        )
        .with_context(|| {
            format!(
                "Failed to install extlinux-compatible configuration extlinux.conf to {:?}",
                &self.esp_paths.extlinux_config
            )
        })?;
