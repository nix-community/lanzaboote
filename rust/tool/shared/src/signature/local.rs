...
impl LanzabooteSigner for LocalKeyPair {
   ...
   fn build_and_sign_stub(&self, stub: &crate::pe::StubParameters) -> Result<Vec<u8>> {
       let working_tree = tempdir()?;
       let lzbt_image_path = lanzaboote_image(&working_tree, stub)
       ...
       Ok(std::fs::read(&to)
           .context("Failed to read a lanzaboote image")?)
   }

   fn verify(&self, pe_binary: &[u8]) -> Result<bool> {
       let working_tree = tempdir()
           .context("Failed to get a temporary working tree")?;
       let from = working_tree.write_secure_file(pe_binary)
