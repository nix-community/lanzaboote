  ...
  testScript =
    ...
    + ''
      ...
      with subtest("pcrlock policy is generated"):
        ...
        with subtest("pcrlock policy contains PCRs 0 and 7"):
          policy = json.loads(policy_json)
          pcrs = [x.get("pcr") for x in policy.get("pcrValues")]
          t.assertIn(0, pcrs)
          t.assertIn(7, pcrs)

        with subtest("pcrlock policy doesn't contain PCR 4 yet"):
      ...
      # with subtest("Install specialisation artifacts"):
      #   print(machine.succeed("/run/current-system/specialisation/new-generation/bin/switch-to-configuration boot"))
      #   print(machine.succeed("tree /var/lib/pcrlock.d"))
      #   machine.succeed("bootctl set-default nixos-generation-1-specialisation-new-generation-\\*.efi")
    '';
}
